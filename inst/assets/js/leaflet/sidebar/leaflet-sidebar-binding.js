...
LeafletWidget.methods.addSidebar = function(options) {
  (function(){
    var map = this;
    if (map.sidebar) {
      map.sidebar.remove();
    ...
    map.sidebar = L.control.sidebar(options);
    map.controls.add(map.sidebar);

        // Event Listeners
    map.sidebar.on('content', function(e) {
      if (!HTMLWidgets.shinyMode) return;
      Shiny.onInputChange(e.id+'_open', e.id);
    });
