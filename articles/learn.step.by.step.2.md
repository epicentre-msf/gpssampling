# Spatial Sampling after building zone identification (Shrink)

> A step-by-step tutorial to do a spatial sampling by building zone
...
the identification of building zone, followed by a spatial sampling.
This has been allowed by the GPS Sampling app.
...
3.  **Sample**: From this ensemble, GPS Sampling can randomly generate a
    sample of the desired size, and this spatial sample of GPS
    coordinates can then be exported to a GPS device for field data
    collection in each of the household living under the sampled
    building zones
...
1.  Select the polygon. You can also observe that the table of results
    appears in the right window. ![](images/table_data_ss.webp)
