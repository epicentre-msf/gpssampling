...
This method consists in combining the analysis of a satellite image for the identification of building zone, followed by a supervised spatial  sampling. This has been allowed by the `r pkg_infos$title` app.
...
3. **Sample**: From this ensemble, `r pkg_infos$title` can randomly generate a unique sample and give you the choice to keep the sample or not, and this spatial sample of GPS coordinates can then be exported to a GPS device for field data collection in each of the household living under the sampled building zones
...
3. Choose `r pkg_infos$title` (1 [ENTER]). After a few seconds, `r pkg_infos$title` is displayed in your browser. Select **spatial sample**.
...
1. Click on the button **Search** (`r gpssampling:::icon_mdi('magnify')`) on the bottom-left. You can now type a location.<br>
...
2. Click on the button **Draw a polygon** (`r gpssampling:::icon_mdi('shape-square-plus')`).
...
5. Click on the button **Save** (`r gpssampling:::icon_mdi('check-bold')`). Confirm with **OK**.

6. Now, you can go to the next step **Identify**

### Uploading an existing polygone

1. Click on the button **Add a polygon layer...** (`r gpssampling:::icon_mdi('layers-plus')`). This dialog box popup.<br>
...
[As in the other method above, do not forget to validate `r gpssampling:::icon_mdi('check-bold')` to be able to proceed to step 2 (tab "Identify")  !]{.red-text}
...
1. To inspect a tile, you can:
    ...
    * Click on the button **Search and identify** (`r gpssampling:::icon_mdi('feature-search-outline')`).
...
3. When all urban zones have been marked in a tile, you can
      ...
      * Click on the button **Check** (`r gpssampling:::icon_mdi('square-rounded', color = "yellow")`).
      * Type [**SPACE**]
...
5. When all tiles of the polygone have been done : click on the button **Valid All** (`r gpssampling:::icon_mdi('checkbox-multiple-marked')`) to validate the pointing of building zones.

::: {.alert .alert-info}
**Notes**:

* In case of mistakes : re-clicking on a building zone allows to remove it.
* It is possible to change the size and color of marks by clicking the button **Settings** (`r gpssampling:::icon_mdi('cog')`).
...
2. Click the button **New Sample...** (`r gpssampling:::icon_mdi('map-marker-radius-outline')`). The map zoom to the random sample point with a radius zone.

    ![](./images/generate_sss_04.webp)

3. Click on the button  **Save** (`r gpssampling:::icon_mdi('map-marker-check-outline')`) if you choose to keep the point.
...
[To be allowed to proceed to the next step, after points have been randomly sampled (by any of the two methods above), click on the icon `r gpssampling:::icon_mdi('check-bold')` ]{.red-text}
...
2. Export the file of sampled points from `r pkg_infos$title`:
    ...
    **Offline** with a GPS  device (Garmin _eTrex_):
    ...
    1. Click on the button **GPX** (`r gpssampling:::icon_mdi('download')`) and save the file in your computer's disk.
    ...
    **Online** with a smartphone with camera:
    ...
    1. Click on the button **GPX** (`r gpssampling:::icon_mdi('qrcode-scan')`). A window opens with a QR code
