...
`sample_communities()` selects points using a greedy algorithm that ensures
no two selected points are closer than `min_distance` meters. The starting
point in each community is the building closest to a road (via OSM).
...
(selected points), `$secondary` (remaining points), `$buildings` (all
candidates), and metadata (`$start_id`, `$min_distance`, `$seed`).
...
| `find_start_points()` | Find closest-to-road building per community |
| `sample_communities()` | Reproducible min-distance sampling across communities |
