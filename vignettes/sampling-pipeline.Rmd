...
`n_required` drawn become primary; the rest become secondary. If the pool
is too small for the full secondary set, all remaining points fill in.
...
If different communities need different team counts, pass a named vector:

```{r batches-named, eval = FALSE}
...
`leaflet_communities()` creates an interactive map with layer toggles,
community navigation, fullscreen, and multiple base maps:
...
- **Z-ordered layers**: communities (bottom) > roads > buildings > buffers > points (top)
- **Grouped toggles**: "Primary Points", "Secondary Points", "Buildings", "Roads"
...
leaflet_communities(
  ...
  buildings_sf = buildings_filtered,
  roads_list = roads,
  out_file = "output/maps/sampling_map.html"
)
```
...
| `sample_communities()` | Random sampling with proximity-based ordering (`joint` option) |
| `fetch_community_roads()` | Pre-download OSM roads per community (with caching) |

### GPS Management

| Function | Purpose |
|----------|---------|
| `split_batches()` | Assign round-robin batch numbers (uniform or per-community) |
...
| `map_community()` | Per-community static map with batch coloring |
...
| `leaflet_communities()` | Interactive leaflet map with layer controls |
