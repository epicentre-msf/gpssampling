---
title: "GPS Sampling Pipeline: A Complete Guide"
subtitle: >
  Reproducible spatial sampling of buildings, GPS point management,
  and mapping -- from raw data to field-ready outputs.
output:
...
vignette: >
  %\VignetteIndexEntry{GPS Sampling Pipeline: A Complete Guide}
...
The `gpssampling` package provides a **programmatic sampling pipeline** for
selecting buildings within community polygons for field epidemiology surveys.
Everything runs from R scripts -- no Shiny app required.

This vignette is a **complete, step-by-step guide** designed for someone who
has never used the package before. It covers every function in the pipeline,
explains what each one does, shows its key parameters, and walks through a
realistic workflow from start to finish.
...
- **Export files** (GPX, GeoPackage, KML) ready for GPS devices and OsmAnd
...
1. **Study area boundary** -- a polygon covering your entire area of interest.
   This is used to download building footprints from OpenStreetMap.
2. **Community polygons** -- one polygon per community you want to sample.
   Each polygon must have a column with a unique community name.

Both can be GeoPackage (`.gpkg`), Shapefile (`.shp`), GeoJSON, or any format
readable by `sf::st_read()`. Any coordinate reference system (CRS) works --
the package reprojects internally as needed.
...
**Important:** the `community_id_col` parameter (used in many functions below)
must match the name of the column that identifies your communities. Throughout
this vignette we use `community_id_col = "name"`. If your column is called
something else (e.g., `"site"`, `"village"`, `"locality"`), adjust accordingly.


# Step 2: Fetch Building Footprints

`fetch_osm_buildings()` downloads building footprints from
[OpenStreetMap](https://www.openstreetmap.org/) for a given area. It uses
tile-based Overpass API queries so that even large areas can be processed
...
- `building` -- the OSM building type tag (e.g., `"yes"`, `"residential"`,
  `"hospital"`, `"school"`)
...
- For very large study areas (spanning many tiles), expect the download to take
  several minutes. The function prints progress per tile.
...
Not all buildings in OSM are residential. `filter_buildings()` removes
non-residential structures like hospitals, schools, and warehouses so you
only sample from buildings where people live.

The function supports **three workflows** depending on your data:

## Path A: OSM-only data (most common)

When your buildings come from `fetch_osm_buildings()`, they have a `building`
column with OSM tags. Filter directly on that column:
...
**Important:** when filtering OSM data, you must set `building_col = "building"`
because the default is `"type"` (designed for user-provided datasets).

## Path B: Your own footprints + OSM labeling

If you have building footprints from another source (e.g., Microsoft Building
Footprints, Google Open Buildings) that do not have type tags, you can use OSM
to label them. The function spatially matches your buildings against OSM
buildings and copies the tags over:
...
By default, unmatched buildings (those with no OSM counterpart) are **kept**,
assuming they are residential. Set `keep_untagged = FALSE` to drop them.
...
If your data has no type information and you do not provide OSM data, the
function returns all buildings as-is with a warning.
...
Add or remove tags to suit your study. For example, to also exclude religious
...
`crop_buildings()` intersects your building footprints with community polygons,
converts each building to its centroid point, and returns a **named list** --
...
1. Communities with the same name are dissolved (e.g., a multipolygon stored
   as separate features with the same name is merged into one).
2. A spatial index (`st_intersects`) finds buildings that overlap each community.
3. Building polygons are converted to centroids.
4. Only centroids that fall **within** the community boundary are kept.
5. Points are sorted deterministically by longitude then latitude, ensuring
   reproducible `id` numbering.


# Step 5: Visualize Building Footprints (Optional but Recommended)

Before sampling, it is helpful to visually verify that your building data looks
correct. `map_cropped_buildings()` renders a map per community showing the
actual building footprint shapes over an OSM basemap.
...
`sample_communities()` is the core function. It selects a specified number of
buildings per community, enforcing a minimum distance between any two selected
points so that field teams cover different areas rather than clustering.
...
1. **Random selection** -- all points (including the first) are chosen at
   random. At each step, the algorithm picks a random candidate from buildings
   that are at least `min_distance` meters away from every already-selected
   point. If no candidates meet the distance constraint, the remaining points
   are drawn randomly with a warning.

2. **Proximity ordering** -- after selection, points are reordered for
   efficient field work using a **nearest-neighbour chain**: the selected point
   closest to an OSM road becomes #1, then the nearest unvisited point becomes
   #2, and so on. This `selection_order` determines the order field workers
   follow.

After primary selection, **secondary (replacement) points** are drawn from the
remaining pool using the same algorithm.
...
If all communities need the same number of points, pass a single integer:
...
| Column | Description |
|--------|-------------|
| `id` | Original building ID from `crop_buildings()` |
...
communities (alphabetically). Secondary point IDs continue from N+1. This
ensures every point across the entire project has a unique identifier,
which is critical for field tracking.

## The summary table

By default (`print_table = TRUE`), `sample_communities()` prints a
`flextable` summary and attaches it to the result:
...
A **TOTAL** row summarizes across all communities. Violations (pairs below
threshold) are highlighted in red.

## Joint sampling (less clustered secondary points)

By default, primary and secondary points are drawn **independently**: first
the primary set is drawn, then the secondary set is drawn from the remaining
pool. This can lead to clustering in the secondary set when the remaining
pool is sparse.
...
With `joint = TRUE`, the algorithm draws up to `2 * n_required` points in one
call, enforcing the minimum distance across **all** of them. The first
`n_required` drawn become primary; the rest become secondary. This produces
better spatial spread across both sets.
...
The algorithm uses `withr::with_seed()` (not `set.seed()`), so it does not
affect your global RNG state. Each community gets a deterministic sub-seed
derived from the master `seed` and the community name, so **adding or removing
a community does not change the selection in other communities**.

## Key parameters

| Parameter | Default | Description |
|-----------|---------|-------------|
| `buildings_list` | *(required)* | Named list of `sf` POINT from `crop_buildings()` |
...
`sample_communities()` downloads roads from OSM for each community to do
the proximity ordering. If you want to pre-download roads (e.g., while you
have internet) and cache them for later use, use `fetch_community_roads()`:
...
Roads are saved as `.gpkg` files, one per community. On subsequent calls,
cached files are reused without re-downloading.

Pass the same `road_dir` to `sample_communities()` to use the cache:
...
`split_batches()` distributes points across field teams using **round-robin
assignment**. Points are assigned in order of their `selection_order`, so
consecutive points (which are geographically close thanks to the nearest-
neighbour chain) go to different batches.
...
Each community's points get an `assigned_batch` column (1, 2, 3, 4, or 5).
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `samples_list` | *(required)* | Output of `sample_communities()` |
...
`create_buffers()` generates circular buffer polygons around sampled points.
Buffers are computed in a local UTM projection (auto-detected) for accurate
metric distances, then reprojected back to WGS84.
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `x` | *(required)* | An `sf` POINT, or a named list from `sample_communities()` / `split_batches()` |
...
`export_points()` returns a tibble listing every file it created:
...
The overlay shows semi-transparent green circles around each sampled point,
helping field workers identify which buildings to visit.

## Buffer building distribution

When `export_points()` is called with `print_table = TRUE`, it attaches
per-buffer building counts to the manifest. Use `plot_buffer_distribution()`
to visualise how buildings are distributed across buffers — useful for
assessing sampling density uniformity.

```{r buffer-distribution, fig.width=8, fig.height=5}
...
```{r buffer-distribution-custom, fig.width=8, fig.height=5}
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `samples_list` | *(required)* | Output of `split_batches()` or `sample_communities()` |
...
`create_earth_project()` generates a KML file organized into folders for
viewing in Google Earth. This is useful for reviewing the sampling plan
on a desktop before going to the field.
...
- **Primary Points** -- with subfolders per community, each point labeled by
  `point_id`
...
Each folder can be toggled on/off in Google Earth. Points and buffers use
distinct colors (configurable).
...
`zip_points()` bundles GPX files and SQLite tile overlays into zip archives,
ready for copying to field devices:
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `export_dir` | *(required)* | Root export directory (same as `out_dir` in `export_points()`) |
...
`email_points()` sends zip archives as email attachments. It uses the
`emayili` package (must be installed separately) and requires SMTP credentials.
...
rendering). All three are in `Suggests` and installed automatically when first
needed.

## Generate all maps at once

`map_all_communities()` is the main function for batch map generation. It
creates:

- An **overview map** showing all communities with uniform point color
- **Per-community primary maps** with batch coloring
- **Per-community secondary maps** with batch coloring (if secondary points
  are provided)
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `primary_batches` | *(required)* | Named list of `sf` from `split_batches()` |
| `communities_sf` | *(required)* | Community polygons |
| `community_id_col` | `"name"` | Column name for community ID |
| `secondary_batches` | `NULL` | Named list of `sf` from `split_batches()` for secondary maps |
...
`map_community()` gives full control over a single map:
...
- Auto-generated subtitle with point count, ID range, and minimum pairwise
  distance
...
| `point_shape` | `16` | Marker shape (see `?points` for options) |
...
`map_overview()` creates a zoomed-out map of all communities:
...
The overview shows all communities with labels at centroids, uniform point
color (no batch coloring), and buffer zones. It is designed to give a
bird's-eye view of the sampling plan.


# Step 15: Interactive Leaflet Map

`leaflet_communities()` creates a fully interactive map with layer toggles,
a community navigation panel, and multiple base map options. This is ideal
for digital review, presentations, and sharing with stakeholders.
...
- **Z-ordered layers:** communities (bottom) > roads > buildings > buffers >
  points (top). Layers never obscure each other.
- **Layer toggles:** checkbox controls for Communities, Roads, Buildings,
  Primary Points, Primary Buffers, Secondary Points, Secondary Buffers.
- **Distinct markers:** circles for primary points, triangles for secondary.
- **Batch coloring:** points colored by `assigned_batch` (using the Set1
  color palette).
- **Community navigation:** a panel with quick-zoom buttons for each community.
- **Multiple base maps:** OpenStreetMap, Light (CartoDB), Satellite (ESRI),
  OSM Humanitarian.
...
- **Buildings toggle:** building footprints are included but hidden by default
  (they can be heavy). Toggle via the layer control.
- **Self-contained HTML:** pass `out_file` to save as a standalone HTML file
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `primary_batches` | *(required)* | Named list from `split_batches()` |
| `communities_sf` | *(required)* | Community polygons |
| `community_id_col` | `"name"` | Column name for community ID |
| `secondary_batches` | `NULL` | Named list from `split_batches()` |
| `buildings_list` | `NULL` | Named list from `crop_buildings()` |
| `roads_list` | `NULL` | Named list from `fetch_community_roads()` |
...
`find_start_points()` is a standalone utility that identifies the building
closest to an OSM road in each community. The main sampling pipeline uses
this logic internally (via `order_selected_points()`), but you can call it
...
| `crop_buildings()` | Intersect buildings with community polygons, return centroids |
| `sample_communities()` | Random sampling with min-distance constraint and proximity ordering |
| `fetch_community_roads()` | Pre-download and cache OSM roads per community |
...
| Function | Purpose |
|----------|---------|
| `split_batches()` | Assign round-robin batch numbers |
...
| `create_earth_project()` | Generate a Google Earth KML with organized folders |
...
| Function | Purpose |
|----------|---------|
| `map_cropped_buildings()` | Building footprint maps per community |
| `map_community()` | Per-community static map with batch coloring and labels |
| `map_overview()` | Zoomed-out static overview of all communities |
| `map_all_communities()` | Generate and optionally save all static maps |
| `leaflet_communities()` | Interactive leaflet map with layer controls and navigation |
| `plot_buffer_distribution()` | Histogram of buildings per buffer from export results |
...
- Reduce the area size or increase the `zoom` level in `fetch_osm_buildings()`
  (more tiles but smaller queries).
- For roads, increase the `timeout` parameter in `fetch_community_roads()`.

## "Requested N points but only M available"

The number of buildings in a community is less than `n_required`. Solutions:

- Reduce `n_required` for that community.
- Check your community polygon -- it may be too small or misaligned.
- Check that `filter_buildings()` did not remove too many buildings.
...
- Reduce `min_distance`.
- Reduce `n_required`.
- This is expected when `n_required` is a large fraction of the available
  buildings.

## Basemap tiles fail to download

If `maptiles::get_tiles()` fails (network issue or provider down), maps
are rendered without a basemap. The data layers (points, buffers, boundaries)
still appear correctly. Try a different `basemap` provider:
...
- Verify the `.sqlitedb` file is in `Android/data/net.osmand/files/tiles/`.
- In OsmAnd, go to Configure map > Overlay (or Underlay) and select the file.
- Make sure the zoom level you are viewing is within the tile range (default
  zoom 8-14).
